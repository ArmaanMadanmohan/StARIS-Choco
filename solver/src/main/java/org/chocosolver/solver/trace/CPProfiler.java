...
public class CPProfiler extends SearchViz {
    ...
    protected boolean connect(String label) {
        if (connector == null) {
            connector = new Connector(6565); // 6565 is the port used by cpprofiler by default
        }
        try {
            connector.connect();
            connector.start("choco", -1, true); // starting a new tree (also used in case of a restart)
        } catch (IOException e) {
